  ...
  async (
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_IDENTITY_API}/auth/token`,
        credentials,
        {
          headers: {
...
export const handleGoogleCallback = createAsyncThunk(
  ...
  async (code: string, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_IDENTITY_API}/auth/outbound/authentication`,
        null,
        { params: { code } }
      );
...
export const logout = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      await axios.post(
        `${import.meta.env.VITE_IDENTITY_API}/auth/logout`,
        { token: getToken() },
        {
          headers: {
